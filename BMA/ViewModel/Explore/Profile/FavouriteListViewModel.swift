...
struct FavoriteListViewModel: FavoriteListDelegate {
    ...
    func removeFav(removeAt: String) {
        for index in 0..<self.favoriteList.value.count {
            if self.favoriteList.value[index].id == removeAt {
                self.favoriteList.value = self.favoriteList.value.filter({ $0.id != removeAt })
            }
        }
    }
}
